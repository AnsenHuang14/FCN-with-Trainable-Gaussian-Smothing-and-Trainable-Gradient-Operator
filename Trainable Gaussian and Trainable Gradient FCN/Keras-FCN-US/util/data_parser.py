...
def load_data(training_path,target_path,index_path,input_shape,input_hist,shuffle,nbins,blur,blur_kernel_shape,sigma):
	...
	for i in range(num):
		...
		# # plt.subplot(1,3,1)
		# 	plt.imshow(img.reshape(360,360),'gray')
		# 	plt.xticks([]), plt.yticks([])
		# 	plt.title('Original image')
		# 	plt.show()
		# 	# plt.subplot(1,3,2)
		# 	plt.bar(bin_range,hist_norm)
		# 	plt.ylabel('Frequency'), plt.xlabel('Gray scale')
		# 	plt.title('Image histogram')
		# 	plt.show()
		# 	# plt.subplot(1,3,3)
		# 	plt.imshow(hist_map.reshape(360,360),'gray')
		# 	plt.xticks([]), plt.yticks([])
...
if __name__ == '__main__':
	X,y,hist = load_data(training_path='../data/all/training',target_path='../data/all/train_target',index_path='../data/all_index.csv',input_shape=360,input_hist=True,shuffle=False,nbins=256,blur=True,blur_kernel_shape=7,sigma=[2])
	print(X.shape)
	X = gradient_input(X,5,grad_type=2)
	print(X.shape)
